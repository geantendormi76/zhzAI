...
def generate_tree(
    ...
    default_dir_excludes = {".git", ".venv", "__pycache__", "node_modules", ".pytest_cache", ".mypy_cache", ".idea", ".vscode", "build", "dist", "*.egg-info"}
...
if __name__ == "__main__":
    project_root_str = input("请输入项目根目录的绝对路径 (例如 /home/zhz/zhz_agent): ")
    project_root = Path(project_root_str)

    if not project_root.is_dir():
        print(f"错误：提供的路径 '{project_root_str}' 不是一个有效的目录。")
    else:
        # --- 自定义排除项 ---
        ...
        # --- 仅包含特定顶层目录 (如果需要) ---
        # 例如，如果我们只关心 'zhz_rag', 'zhz_rag_pipeline_dagster', 'core'
        # dir_to_include_at_root = ["zhz_rag", "zhz_rag_pipeline_dagster", "core", "scripts"]
        # top_level_dirs = [d for d in project_root.iterdir() if d.is_dir() and d.name in dir_to_include_at_root]
        
        # 如果不过滤顶层目录，则直接使用 project_root
        top_level_dirs = [project_root] # 或者使用上面的过滤逻辑

        all_output_lines = [f"项目结构树: {project_root.resolve()}"]

        for top_dir_path in top_level_dirs:
            if top_dir_path != project_root: # 如果是子目录，显示其名称
                 all_output_lines.append(f"\nDirectory: {top_dir_path.name}/")
